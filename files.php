...
$url_files = prepareUrl("index.php?site=files");
$url_savefile = prepareUrl("index.php?site=files&action=save");
$url_saveedit = prepareUrl("index.php?site=files&action=saveedit");
...
if($action=="save") {
	if(!isfileadmin($userID)) die(redirect(prepareUrl("index.php?site=files"), $_language->module['no_access'], "3"));
	...
	if($upfile or $fileurl) {
		...
		if(safe_query($qry)) {
			redirect(prepareUrl("index.php?site=files&file=".mysql_insert_id().""), $_language->module['file_created'], "3");
		}
		else redirect(prepareUrl("index.php?site=files"), $_language->module['file_not_created'], "3");
	}
	else redirect(prepareUrl("index.php?site=files"), $_language->module['no_valid_file'], "3");
}
...
elseif($action == "delete") {
  ...
  if($cat) $ref = '&cat=' . $cat;
	...
	if($file) {
		...
		if(safe_query("DELETE FROM ".PREFIX."files WHERE fileID='$file'")) redirect(prepareUrl("index.php?site=files") . $ref, $_language->module['file_deleted'], "3");
		else redirect(prepareUrl("index.php?site=files"), $_language->module['file_not_deleted'], "3");
	}
...
elseif($action=="newfile") {
	...
	$adminactions .= '<input type="button" class="btn btn-primary"  onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=files').'\');return document.MM_returnValue" value="'.$_language->module['files'].'" /> ';
	if(isfileadmin($userID)) {
		$adminactions .= '</td><td align="right"><input type="button" class="btn btn-primary"  onclick="MM_openBrWindow(\'admin/admincenter.php?site=filecategorys\',\'\')" value="'.$_language->module['new_category'].'" /></td></tr></table>';
		...
		if($filecats == '') redirect(prepareUrl('index.php?site=files'), $_language->module['first_create_file-category'], '3');
		else {
			echo $adminactions;
			$url_newfile = prepareUrl("");
			eval("\$files_new = \"".gettemplate("files_new")."\";");
			echo $files_new;
		}
	}
	else redirect(prepareUrl("index.php?site=files"), $_language->module['no_access'], "3");
}
elseif($action=="edit") {
	$fileID = $_GET['fileID'];
	if($fileID) {
		if(isfileadmin($userID)) {
			// ADMINACTIONS
			$adminactions = '<table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td align="right"><input type="button" class="btn btn-primary"  onclick="MM_openBrWindow(\'admin/admincenter.php?site=filecategorys\',\'\')" value="'.$_language->module['new_category'].'" /></td></tr></table><br />';
...
elseif(isset($_GET['cat'])) {
	...
	if(isfileadmin($userID)) {
		$adminactions = '<table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td align="left">';
		$adminactions .= '</td><td align="right"><input type="button" class="btn btn-primary"  onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=files&action=newfile').'\');return document.MM_returnValue" value="'.$_language->module['new_file'].'" /> ';
		$adminactions .= '<input type="button" class="btn btn-primary"  onclick="MM_openBrWindow(\'admin/admincenter.php?site=filecategorys\',\'\')" value="'.$_language->module['new_category'].'" />';
	...
	while($cat_id != 0) {
		...
		$category = "<a href=\"".prepareUrl("index.php?site=files&cat=".$subcat['filecatID'])."\" class=\"titlelink\">".$subcat['name']."</a> &raquo; ".$category;
	...
	if(mysql_num_rows($subcats)) {
		...
		while($subcat = mysql_fetch_array($subcats)) {
			...
			$catname = '<a href="'.prepareUrl('index.php?site=files&cat='.$subcat['filecatID'].'').'"><b>'.$subcat['name'].'</b></a>';
			$downloads = 0;
	...
	if(mysql_num_rows($files)) {
		...
		while($file = mysql_fetch_array($files)) {
			...
			$fileid = $file['fileID'];
			$filename = '<a href="'.prepareUrl('index.php?site=files&file='.$fileid.'').'"><b>'.clearfromtags($file['filename']).'</b></a>';
...
elseif(isset($_GET['file'])) {
	...
	$reportlink = '<a href="'.prepareUrl('index.php?site=files&action=report&link='.$file['fileID'].'').'"><b>'.$_language->module['report_dead_link'].'</b></a>';
	$date = date("d.m.Y", $file['date']);

	// FILE-AUTHOR
	$uploader = cleartext('[flag]'.getcountry($file['poster']).'[/flag]').' <a href="'.prepareUrl('index.php?site=profile&id='.$file['poster'].'').'">'.getnickname($file['poster']).'</a>';

	// FILE-CATEGORY
	$cat = mysql_fetch_array(safe_query("SELECT * FROM ".PREFIX."files_categorys WHERE filecatID='".$file['filecatID']."'"));
	$category = '<a href="'.prepareUrl('index.php?site=files&cat='.$cat['filecatID'].'').'" class="titlelink">'.$cat['name'].'</a>';
	$categories = '<a href="'.prepareUrl('index.php?site=files&cat='.$cat['filecatID'].'').'"><b>'.$cat['name'].'</b></a>';

	$cat_id = $cat['subcatID'];
	while($cat_id != 0) {
		$subcat = mysql_fetch_array(safe_query("SELECT filecatID, name, subcatID FROM ".PREFIX."files_categorys WHERE filecatID='".$cat_id."'"));
		$category = "<a href=\"".prepareUrl("index.php?site=files&cat=".$subcat['filecatID'])."\" class=\"titlelink\">".$subcat['name']."</a> >> ".$category;
		$categories = "<a href=\"".prepareUrl("index.php?site=files&cat=".$subcat['filecatID'])."\"><b>".$subcat['name']."</b></a> >> ".$categories;
		$cat_id = $subcat['subcatID'];
	}
	...
	if(isfileadmin($userID)) {
		...
		$admintools .= '<input type="button" class="btn btn-primary"  onclick="MM_goToURL(\'parent\', \''.prepareUrl('index.php?site=files&action=edit&fileID='.$file['fileID'].'').'\');return document.MM_returnValue" value="'.$_language->module['edit_file'].'" /> ';
		$admintools .= '<input type="button" class="btn btn-primary"  onclick="MM_confirm(\''.$_language->module['really_delete_file'].'\', \'index.php?site=files&action=delete&file='.$file['fileID'].'\')" value="'.$_language->module['delete_file'].'" /> ';
		$admintools .= '</td></tr>';
	}
	...
	else $link = $_language->module['download_registered_only'].'<br /><a href="'.prepareUrl('index.php?site=login').'">'.$_language->module['login'].'</a> | <a href="'.prepareUrl('index.php?site=register').'">'.$_language->module['register'].'</a>';
...
elseif($action=="report") {
	...
	$referer = $hp_url.'/'.prepareUrl('index.php?site=files&fileID='.$id);
	...
	if(isfileadmin($userID)) {
		...
		$adminactions .= '<input type="button" class="btn btn-primary"  onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=files&action=newfile').'\');return document.MM_returnValue" value="'.$_language->module['new_file'].'" /> ';
		$adminactions .= '<input type="button" class="btn btn-primary"  onclick="MM_openBrWindow(\'admin/admincenter.php?site=filecategorys\',\'\')" value="'.$_language->module['new_category'].'" />';
		$adminactions .= '</td></tr></table><br />';
	}
	...
	if($totalcats) {
		...
		if($totalfiles) {
			...
			$lastfile = '<a href="'.prepareUrl('index.php?site=files&file='.$filedata['fileID'].'').'" title="'.$filedata['filename'].' - '.str_replace('%d', $filedata['rating'], $_language->module['rating_x_of_10']).'">'.$filename.'</a>';
		}
		else {
			$traffic = 'n/a';
			$hddspace = 'n/a';
		...
		while($file = mysql_fetch_array($top5qry)) {
			...
			$filename = '<a href="'.prepareUrl('index.php?site=files&file='.$file['fileID'].'').'"><b>'.$filename.'</b></a>';
			if($file['downloads'] != '0') {
				$top5 .= '<tr>
    ...
    eval("\$files_stats = \"".gettemplate("files_stats")."\";");
		...
		if($totalcats) {
			...
			while($cat = mysql_fetch_array($catQry)) {
				...
				$catname = '<a href="'.prepareUrl('index.php?site=files&cat='.$catID.'').'"><b>'.$cat['name'].'</b></a>';
				...
				if($catFileTotal OR $subcategories) {
					...
					$lastfile_cat = '<a href="'.prepareUrl('index.php?site=files&file='.$filedata['fileID'].'').'" title="'.$filedata['filename'].' - '.sprintf($_language->module['rating_x_of_10'], $filedata['rating']).'">'.$filename.'</a>';
