...
$form_url = prepareUrl("index.php?site=register");
...
if(isset($_POST['save'])) {
	...
	if(!$loggedin){
	  ...
	  // check captcha
		...
		// GOOGLE reCAPTCHA
		if(!$isHuman){$error[]=$_language->module['wrong_securitycode'];}
	  	...
	  	if(count($error)) {
			...
			$activationlink='http://'.$hp_url.'/'.prepareUrl('index.php?site=register&key='.$activationkey);
		...
		redirect(prepareUrl("index.php?site=register"),str_replace('%pagename%',$GLOBALS['hp_title'],$_language->module['no_register_when_loggedin']),3);
	}
}
if(isset($_GET['key'])) {

	safe_query("UPDATE `".PREFIX."user` SET activated='1' WHERE activated='".$_GET['key']."'");
	if(mysql_affected_rows()) redirect(prepareUrl('index.php?site=login'),$_language->module['activation_successful'],3);
	else redirect(prepareUrl('index.php?site=login'),$_language->module['wrong_activationkey'],3);
...
elseif(isset($_GET['mailkey'])) {
  if(mb_strlen(trim($_GET['mailkey']))==32){
		...
		if(mysql_affected_rows()) redirect(prepareUrl('index.php?site=login'),$_language->module['mail_activation_successful'],3);
		else redirect(prepareUrl('index.php?site=login'),$_language->module['wrong_activationkey'],3);
  }
}
else {
