<?php
	...
	if(isset($_GET['action'])){
		...
		// SHOWS PLUGIN LIST
		$plugin_class->view_template("admin_plugins", "list_header");
		...
		if(count($all_plugins)>0){
			foreach($all_plugins as $plugin){
				...
				$plugin_class->view_template("admin_plugins", "list_content", $variables);
			}
		}else{
			$plugin_class->view_template("admin_plugins", "no_list_content");
		}
		$plugin_class->view_template("admin_plugins", "list_footer");
	}
?>