...
elseif(isset($_GET['action']) and $_GET['action'] == "edit") {
  ...
  while($ds=mysql_fetch_array($ergebnis)) {
    ...
    if($ds['accesslevel'] == 2) $accesslevel = $_language->module['clanmember_only'];
      ...
      <td class="'.$td.'"><a href="../'.prepareUrl('index.php?site=static&staticID='.$ds['staticID'].'').'" target="_blank">'.getinput($ds['name']).' - URL Encrypted</a></br>
	  <a href="../'.('index.php?site=static&staticID='.$ds['staticID'].'').'" target="_blank">'.getinput($ds['name']).' - webSPELL URL</a>
	  </td>
