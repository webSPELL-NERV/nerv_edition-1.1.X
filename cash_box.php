...
if(isset($_POST['save']) and $_POST['save']) {
	...
	header("Location: ".prepareUrl("index.php?site=cash_box&id=$id"));
}
...
elseif(isset($_GET['delete']) and $_GET['delete']) {
	...
	header("Location: ".prepareUrl("index.php?site=cash_box"));
}
	...
	if(isset($_GET['action']) and $_GET['action']=="new") {
        ...
        <tr><td height="15"></td></tr>';
			...
			if(mysql_num_rows($ergebnis)) {
				...
				if(iscashadmin($userID)) $adminaction='<input type="button" onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=cash_box&action=edit&id='.$id.'').'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" /> <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'cash_box.php?delete=true&id='.$id.'\')" value="'.$_language->module['delete'].'" />';
				...
				while($ds=mysql_fetch_array($all)) {
					echo'&#8226; <a href="'.prepareUrl('index.php?site=cash_box&id='.$ds['cashID'].'&squad='.$squadID.'').'"><b>'.$ds['usedfor'].'</b></a><br />';
				}
      ...
      echo'</td></tr></table>';
		...
		if(iscashadmin($userID)) $cashadmin='<input type="button" onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=cash_box&action=new').'\');return document.MM_returnValue" value="'.$_language->module['add_payment'].'" />';
