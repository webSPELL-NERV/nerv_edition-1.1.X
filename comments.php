...
if(isset($_POST['savevisitorcomment'])) {
	...
	$splitted_referer = split('_secret=', $_POST['referer']);
	
	$_POST['referer'] = "index.php?".decryptStringArray($splitted_referer[1]);
	
	if(in_array(trim($name), $nicks)) header("Location: ".prepareUrl($_POST['referer']."&error=nickname")."#addcomment");
	elseif(!$isHuman) header("Location: ".prepareUrl($_POST['referer']."&error=captcha")."#addcomment");
	elseif(checkCommentsAllow($type,$parentID) == false ) header("Location: ".prepareUrl($_POST['referer']));
	else {
		$date=time();
		...
		header("Location: ".prepareUrl($_POST['referer']));
	}
}
	...
	if(isfeedbackadmin($userID) or iscommentposter($userID,$id)) {
		if(!empty($id)) {
			...
			if(mysql_num_rows($dt)) {
				...
				$poster='<a href="'.prepareUrl('index.php?site=profile&id='.$ds['userID'].'').'"><b>'.getnickname($ds['userID']).'</b></a>';
...
elseif(isset($_POST['saveeditcomment'])) {
	...
	if($gesamt) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			if($ds['userID']) {
				$ip='';
				$poster='<a class="titlelink" href="'.prepareUrl('index.php?site=profile&id='.$ds['userID'].'').'"><b>'.strip_tags(getnickname($ds['userID'])).'</b></a>';
