...
$faq_url = prepareUrl("index.php?site=faq");

if($action=="faqcat" and is_numeric($_GET['faqcatID'])) {
	if(ispageadmin($userID)) echo'<input type="button" class="btn btn-primary"  onclick="MM_openBrWindow(\'admin/admincenter.php?site=faq\',\'News\',\'toolbar=yes,status=yes,scrollbars=yes,resizable=yes,width=800,height=600\')" value="'.$_language->module['admin_button'].'" /><br /><br />';
	...
	if(mysql_num_rows($faqcat)) {
		...
		while($ds=mysql_fetch_array($faqcat)){
			...
			$sort=$ds['sort'];
			$question='<a href="'.prepareUrl('index.php?site=faq&action=faq&faqID='.$ds['faqID'].'&faqcatID='.$faqcatID.'').'">'.$ds['question'].'</a>';
		...
		echo str_replace('%faq_url%', $faq_url, $_language->module['no_faq']);
	} 
}
...
elseif($action=="faq") {
	...
	if(mysql_num_rows($faq)) {
		...
		$backlink='<a href="'.prepareUrl('index.php?site=faq&action=faqcat&faqcatID='.$faqcatID.'').'" class="titlelink"><b>'.$faqcatname.'</b></a>';
	...
	else echo str_replace('%faq_url%', $faq_url, $_language->module['no_faq']);
}
	...
	if($anzcats) {
		...
		while($ds=mysql_fetch_array($faqcats)) {
			...
			$faqcatname='&#8226; <a href="'.prepareUrl('index.php?site=faq&action=faqcat&faqcatID='.$ds['faqcatID'].'').'"><b>'.$ds['faqcatname'].'</b></a>';
