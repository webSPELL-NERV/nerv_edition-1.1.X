...
$url_contact_form = prepareUrl("index.php?site=contact");
...
if($action == "send") {
  ...
  if($userID) {
		...
		if(!$isHuman) $fehler[] = $_language->module['wrong_securitycode'];
		else $run=1;
	}
  ...
  if(!count($fehler) and $run) {
		...
		redirect(prepareUrl('index.php?site=contact'),$_language->module['send_successfull'],3);
		unset($_POST['name']);
		unset($_POST['from']);
		unset($_POST['text']);
		unset($_POST['subject']);
