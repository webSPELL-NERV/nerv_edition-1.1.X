...
if(isset($_POST['referer'])){
	$splitted_referer = split('_key=', $_POST['referer']);
	$_POST['referer'] = "index.php?".decryptStringArray($splitted_referer[1]);
}
...
if($userID && !isset($_GET['userID']) && !isset($_POST['userID'])) {
	...
	$username='<a href="'.prepareUrl('index.php?site=profile&id='.$userID.'').'">'.getnickname($userID).'</a>';
	...
	if(mysql_num_rows($topics)) {
		...
		while($db=mysql_fetch_array($topics)) {
          ...
          <table width="100%" cellpadding="1" cellspacing="1">
            <tr>
              <td colspan="3"><a href="'.prepareUrl('index.php?site=forum_topic&topic='.$db['topicID'].'').'">'.$posttime.'<br /><b>'.str_break(getinput($db['topic']), 34).'</b></a><br /><i>'.$db['views'].' '.$_language->module['views'].' - '.$db['replys'].' '.$_language->module['replys'].'</i></td>
            </tr>
          </table>
          </td>
        </tr>';
  ...
  if(mysql_num_rows($posts)) {
      ...
      $postlist.='<tr bgcolor="'.$bgcolor1.'">
          ...
          <table width="100%" cellpadding="2" cellspacing="1">
            <tr>
              <td colspan="3"><a href="'.prepareUrl('index.php?site=forum_topic&topic='.$db['topicID'].'').'">'.$posttime.' <br /><b>'.str_break(getinput($db['topic']), 34).'</b></a></td>
            </tr>
            <tr><td></td></tr>
            <tr>
              <td width="1%">&nbsp;</td>
...
if(isclanmember($userID)) $cashboxpic = '<td style="text-align: center"><a href="'.prepareUrl('index.php?site=cash_box').'"><i class="fa fa-money fa-5x"></i></a><br><span>'.$_language->module['cashbox'].'</span></td>
  <td width="10"></td>';
	else $cashboxpic = '<td></td><td></td>';
  
	if(isanyadmin($userID)) $admincenterpic = '<td style="text-align: center"><a href="admin/admincenter.php" target="_blank"><i class="fa fa-fort-awesome fa-5x"></i></a><br><span>'.$_language->module['adminmenu'].'</span></td>
  <td width="10"></td>';
	else $admincenterpic = '<td></td><td></td>';
  
	if(isset($_SESSION['referer'])) {
		$referer_uri = '<tr><td bgcolor="'.$bgcat.'"><br /><a href="'.prepareUrl($_SESSION['referer']).'" style="padding:8px 0;"><b>&laquo; '.$_language->module['back_last_page'].'</b></a><br />&nbsp;</td></tr>';
				...
				while($ds=mysql_fetch_array($ergebnis)) {
					...
					if(mysql_num_rows($anmeldung)) {
						...
						while ($da = mysql_fetch_array($anmeldung)) {
							...
							if($i>1) $players.=', <a href="'.prepareUrl('index.php?site=profile&id='.$da['userID'].'').'"><font color="'.$fontcolor.'">'.strip_tags(stripslashes(getnickname($da['userID']))).'</font></a>';
							else $players.='<a href="'.prepareUrl('index.php?site=profile&id='.$da['userID'].'').'"><font color="'.$fontcolor.'">'.strip_tags(stripslashes(getnickname($da['userID']))).'</font></a>';
							$i++;
						}
					...
					$clanwars .= '<tr>
						...
						<td bgcolor="'.$bg.'"><a href="'.prepareUrl('index.php?site=calendar&action=announce&upID='.$ds['upID'].'&tag='.$tag.'&month='.$monat.'&year='.$yahr.'').'#event">'.$_language->module['click'].'</a></td>
					</tr>';
					$n++;
				}
	...
	if($anz) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			$events.='<tr>
				...
				<td bgcolor="'.$bg.'"><a href="'.prepareUrl('index.php?site=calendar&tag='.date('d',$ds['date']).'&month='.date('m',$ds['date']).'&year='.date('Y',$ds['date']).'').'#event">'.$_language->module['click'].'</a></td>
			</tr>';
			$n++;
		}
	}
	...
	$url_buddy 			= prepareUrl("index.php?site=buddys");
