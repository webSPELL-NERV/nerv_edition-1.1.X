...
function codereplace($content) {
	...
	for($i=0;$i<$anz;$i++){
		...
		if(strtolower($splits[$i]) == "[code]"){
			...
			if($match){
				$splits[$i] = '<div style="width:100%;height:100%;overflow:auto;background-color:'.$bg1.';color: #333 !important;" class="code">
				<b>'.$_language->module['code'].':</b><hr /><div class="codeinner">';
...
function insideCode($content){
	...
	<tr><td width="5"><div style="text-align: right;">'.$codelines.'</div></td><td valign="top">'.$codecontent.'</td></tr></table>';
...
function youtubereplace($content){
    ...
    function($m) {
		...
		return '<iframe width="100%" height="315" src="https://www.youtube.com/embed/'.$query_params['v'].'" frameborder="0" allowfullscreen></iframe>';
...
function gistreplace($content){
    ...
    "(\[gist\](.*?)\[/gist\])is",
    function($m) {
		// https://gist.github.com/anonymous/0c7786f55d1ed506880b8dbf3fc4fabc
		$url = $m[1];
		// $query_str = parse_url($url, PHP_URL_QUERY);
		// parse_str($query_str, $query_params);	
		$gist = explode('/', $url);
		$pre_gist = $gist[0]."/".$gist[1]."/".$gist[2]."/".$gist[3];
		$gist = $gist[4];
		return '<script src="'.$pre_gist."/".$gist.'.js"></script>';
		//return '<iframe width="100%" height="315" src="https://www.youtube.com/embed/'.$query_params['v'].'" frameborder="0" allowfullscreen></iframe>';
       // return "<div class=\"otsection\" id=\"ots".$id."\">".$m[1]."</div>";
    },
    $content);
