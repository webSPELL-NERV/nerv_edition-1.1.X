...
function redirect($url, $info, $time=1) {
  ...
  <br /><p class="redirect">'.$info.'</p><br /><br />';
}
...
function getmoderators($boardID) {
	...
	while($dm=mysql_fetch_array($moderatoren)) {
		...
		if($j>1) $moderators .= ', <a href="'.prepareUrl('index.php?site=profile&id='.$dm['userID'].'').'">'.$username.'</a>';
		else $moderators = '<a href="'.prepareUrl('index.php?site=profile&id='.$dm['userID'].'').'">'.$username.'</a>';
		$j++;
	}
	return $moderators;
}
