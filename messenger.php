...
$url_outgoing 			= prepareUrl("index.php?site=messenger&action=outgoing");
$url_newmessage			= prepareUrl("index.php?site=messenger&action=newmessage");
...
if(isset($_POST['delete'])) {
	...
	header("Location: ".prepareUrl("index.php?site=messenger&action=outgoing")."");
}
elseif(isset($_POST['quickaction'])) {
	...
	if(isset($_POST['messageID'])){
		...
		header("Location: ".prepareUrl("index.php?site=messenger&action=incoming"));
	}
	else{
		header("Location: ".prepareUrl("index.php?site=messenger&action=incoming"));
	}
...
elseif(isset($_POST['send'])) {
	...
	if($touser[0] != "" AND isset($userID)) {
		...
		header("Location: ".prepareUrl("index.php?site=messenger&action=newmessage"));
		exit();
	}
}
...
elseif($userID) {
	...
	if($action=="incoming") {
		...
		if($pages>1) $page_link = makepagelink(prepareUrl("index.php?site=messenger&action=incoming&sort=$sort&type=$type&entries=$max"), $page, $pages);
		else $page_link='';
    ...
    if($type == "ASC") $sorter='<a href="'.prepareUrl('index.php?site=messenger&action=incoming&page='.$page.'&sort='.$sort.'&type=DESC&entries='.$max.'').'">'.$_language->module['sort'].'</a> <img src="images/icons/asc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
    else $sorter='<a href="'.prepareUrl('index.php?site=messenger&action=incoming&page='.$page.'&sort='.$sort.'&type=ASC&entries='.$max.'').'">'.$_language->module['sort'].'</a> <img src="images/icons/desc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
    ...
    $anz=mysql_num_rows($ergebnis);
      ...
      $n=1;
			while($ds=mysql_fetch_array($ergebnis)) {
				...
				$sender='<a href="'.prepareUrl('index.php?site=profile&id='.$ds['fromuser'].'').'"><b>'.getnickname($ds['fromuser']).'</b></a>';
				...
				$title='&#8226; <a href="'.prepareUrl('index.php?site=messenger&action=show&id='.$ds['messageID'].'').'">'.$title.'</a>';
	...
	elseif($action=="outgoing") {
		...
		if($pages>1) $page_link = makepagelink(prepareUrl("index.php?site=messenger&action=outgoing&entries=$max"), $page, $pages);
		else $page_link='';
    ...
    if($type == "ASC") $sorter='<a href="'.prepareUrl('index.php?site=messenger&action=outgoing&page='.$page.'&sort='.$sort.'&type=DESC&entries='.$max.'').'">'.$_language->module['sort'].'</a> <img src="images/icons/asc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
    else $sorter='<a href="'.prepareUrl('index.php?site=messenger&action=outgoing&page='.$page.'&sort='.$sort.'&type=ASC&entries='.$max.'').'">'.$_language->module['sort'].'</a> <img src="images/icons/desc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
    ...
    echo $pm_outgoing_head;
      ...
      $n=1;
			while($ds=mysql_fetch_array($ergebnis)) {
				...
				$receptionist='<a href="'.prepareUrl('index.php?site=profile&id='.$ds['touser'].'').'"><b>'.getnickname($ds['touser']).'</b></a>';
	...
	elseif($action=="show") {
		...
		else redirect(prepareUrl('index.php?site=messenger'),'',0);
	}
