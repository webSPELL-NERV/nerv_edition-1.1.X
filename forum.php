...
function forum_stats() {
	...
	while($db=mysql_fetch_array($ergebnis)) {
		$n++;
		$years=$db['age'];
		if($n>1) $birthdays.=', <a href="'.prepareUrl('index.php?site=profile&id='.$db['userID'].'').'"><b>'.$db['nickname'].'</b></a> ('.$years.')';
		else $birthdays='<a href="'.prepareUrl('index.php?site=profile&id='.$db['userID'].'').'"><b>'.$db['nickname'].'</b></a> ('.$years.')';
	}
	...
	while($db=mysql_fetch_array($ergebnis)) {
		$n++;
		$years=$db['age'];
		if($n>1) $birthweek.=', <a href="'.prepareUrl('index.php?site=profile&id='.$db['userID'].'').'"><b>'.$db['nickname'].'</b></a> ('.$years.')';
		else $birthweek='<a href="'.prepareUrl('index.php?site=profile&id='.$db['userID'].'').'"><b>'.$db['nickname'].'</b></a> ('.$years.')';
	}
	...
	$newestmember='<a href="'.prepareUrl('index.php?site=profile&id='.$dn['userID'].'').'"><b>'.$dn['nickname'].'</b></a>';
...
function boardmain() {
	...
	while($dk=mysql_fetch_array($kath)) {
		$kathname = "<a href='".prepareUrl("index.php?site=forum&cat=".$dk['catID']."")."'>".$dk['name']."</a>";
		...
		while($db=mysql_fetch_array($boards)) {
			...
			$anzposts=$db['posts'];
			$boardname = $db['name'];
			$boardname ='&#8226; <a href="'.prepareUrl('index.php?site=forum&board='.$board.'').'"><b>'.$boardname.'</b></a>';
			...
			while($lp = mysql_fetch_assoc($q)) {
				...
				if($n == 1) {
					...
					$poster='<a href="'.prepareUrl('index.php?site=profile&id='.$lp['lastposter'].'').'">'.getnickname($lp['lastposter']).'</a>';
					if(isclanmember($lp['lastposter'])) $member=' <img src="images/icons/member.gif" alt="'.$_language->module['clanmember'].'" />';
					else $member='';
					$topic=$lp['topicID'];
					$postlink=prepareUrl('index.php?site=forum_topic&topic='.$topic.'&type=ASC&page='.ceil(($lp['replys']+1)/$maxposts));
			...
			/* FONT AWESOME */
			if($found) $icon='<i class="fa fa-comment forum-icon forum-icon-new"></i>';
			else $icon='<i class="fa fa-comment-o forum-icon forum-icon-nonew"></i>';
	...
	while($db=mysql_fetch_array($boards)) {
		...
		$boardname = $db['name'];
		$boardname='&#8226; <a href="'.prepareUrl('index.php?site=forum&board='.$db['boardID'].'').'"><b>'.$boardname.'</b></a>';
		...
		if($moderators) $moderators=$_language->module['moderated_by'].': '.$moderators;
			...
			/* FONT AWESOME */
			if($found) $icon='<i class="fa fa-comment forum-icon-new" style="font-size: 18px;margin-bottom: 12px;"></i>';
			else $icon='<i class="fa fa-comment-o forum-icon-nonew" style="font-size: 18px;margin-bottom: 12px;"></i>';
...
function showboard($board) {
	...
	$page_link = '';
	if($pages>1) $page_link = makepagelink(prepareUrl("index.php?site=forum&board=$board"), $page, $pages);
	...
	if($db['readgrps'] != "" and !$usergrp and !$ismod) {
		...
		if(!$usergrp){
			...
			redirect(prepareUrl('index.php?site=forum'),'',2);
			return;
		}
	}
	...
	$url_forum 		= prepareUrl("index.php?site=forum");
	$url_forumCat 	= prepareUrl("index.php?site=forum&cat=$cat");
	$url_forumBoard	= prepareUrl("index.php?site=forum&board=$board");
	...
	if($anztopics) {
		...
		while($dt=mysql_fetch_array($topics)) {
			...
			$topicpage_link = '';
			if($topicpages>1) $topicpage_link = makepagelink(prepareUrl("index.php?site=forum_topic&topic=".$dt['topicID']), 1, $topicpages);
			...
			// FONT AWESOME
			if($dt['sticky']) {
				$onicon 		= '<i class="fa fa-sticky-note forum-icon forum-icon-sticky"></i>';
				$officon 		= '<i class="fa fa-sticky-note-o forum-icon forum-icon-sticky-o"></i>';
				$onhoticon 		= '<i class="fa fa-sticky-note forum-icon forum-icon-sticky"></i>';
				$offhoticon 	= '<i class="fa fa-sticky-note-o forum-icon forum-icon-sticky-o"></i>';
			}
			else {
				$onicon 		= '<i class="fa fa-comment forum-icon forum-icon-comment"></i>';
				$officon 		= '<i class="fa fa-comment-o forum-icon forum-icon-comment-o"></i>';
				$onhoticon		= '<i class="fa fa-comment forum-icon forum-icon-comment"></i>';
				$offhoticon 	= '<i class="fa fa-comment-o forum-icon forum-icon-comment-o"></i>';
			}
			
			if($dt['closed']) $folder='<i class="fa fa-lock forum-icon forum-icon-sticky"></i>';
			elseif($dt['moveID']) $folder='<i class="fa fa-external-link-square forum-icon forum-icon-sticky"></i>';
			elseif($userID) {
			...
			$poster='<a href="'.prepareUrl('index.php?site=profile&id='.$dt['userID'].'').'">'.getnickname($dt['userID']).'</a>';
			...
			if($dt['moveID']) { // MOVED TOPIC
				...
				$lastposter='<a href="'.prepareUrl('index.php?site=profile&id='.$dm['lastposter'].'').'">'.getnickname($dm['lastposter']).'</a>';
				if(isclanmember($dm['lastposter'])) $member=' <img src="images/icons/member.gif" alt="'.$_language->module['clanmember'].'" />';
				else $member='';
				$link='<a href="'.prepareUrl('index.php?site=forum_topic&topic='.$dt['moveID'].'').'"><b>'.$_language->module['moved'].': '.$topictitle.'</b></a>';
				...
				$lastposter='<a href="'.prepareUrl('index.php?site=profile&id='.$dt['lastposter'].'').'">'.getnickname($dt['lastposter']).'</a>';
				if(isclanmember($dt['lastposter'])) $member=' <img src="images/icons/member.gif" alt="'.$_language->module['clanmember'].'" />';
				else $member='';
				$link='<a href="'.prepareUrl('index.php?site=forum_topic&topic='.$dt['topicID'].'').'"><b>'.$topictitle.'</b></a>';
			}
...
if(isset($_POST['submit']) || isset($_POST['movetopic']) || isset($_GET['addtopic']) || isset($_POST['addtopic']) || (isset($_GET['action']) and $_GET['action'] == "admin-action") || isset($_POST['admaction'])) {
	...
	if($_POST['admaction']=="closetopic") {
		...
		header("Location: ".prepareUrl('index.php?site=forum&board=$board')."");
	}
	...
	elseif($_POST['admaction']=="delposts") {
		...
		if($deleted) header("Location: ".prepareUrl("index.php?site=forum&board=$board"));
		else header("Location: ".prepareUrl("index.php?site=forum_topic&topic=$topicID"));
	}
	elseif(isset($_POST['movetopic'])) {
		...
		header("Location: ".prepareUrl("index.php?site=forum&board=$toboard"));
	}
	...
	elseif(isset($_POST['newtopic']) && !isset($_POST['preview'])) {
		...
		if(boardexists($board)){
			...
			header("Location: ".prepareUrl("index.php?site=forum"));
		}		
	}
	elseif(isset($_REQUEST['addtopic'])) {
		...
		if($loggedin) {
			if(isset($_POST['preview'])) {
				...
				$username='<a href="'.prepareUrl('index.php?site=profile&id='.$userID.'').'"><b>'.getnickname($userID).'</b></a>';
