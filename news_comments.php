...
$post = "";
if(isnewswriter($userID)) $post='<input class="btn btn-success btn-xs" type="button" onclick="MM_openBrWindow(\'news.php?action=new\',\'News\',\'toolbar=no,status=no,scrollbars=yes,width=800,height=600\')" value="'.$_language->module['post_news'].'" />';
echo $post.' <input type="button" class="btn btn-primary btn-xs" onclick="MM_goToURL(\'parent\',\''.prepareUrl('index.php?site=news&amp;action=archive').'\');return document.MM_returnValue" value="'.$_language->module['news_archive'].'" /><hr />';
...
if($newsID) {
	...
	if($ds['intern'] <= isclanmember($userID) && ($ds['published'] || (isnewsadmin($userID) || (isnewswriter($userID) and $ds['poster'] == $userID)))) {
		...
		foreach($message_array as $val) {
			if($showlang!=$i)	$langs.='<span style="padding-left:2px"><a href="'.prepareUrl('index.php?site=news_comments&newsID='.$ds['newsID'].'&lang='.$val['lang'].'').'"><img src="images/flags/'.$val['countryShort'].'.gif" width="18" height="12" border="0" alt="'.$val['country'].'" /></a></span>';
			$i++;
		}
    ...
    if(isnewsadmin($userID) or (isnewswriter($userID) and $ds['poster'] == $userID)) {
			$adminaction='<input type="button" class="btn btn-warning btn-xs" onclick="MM_openBrWindow(\'news.php?action=edit&amp;newsID='.$ds['newsID'].'\',\'News\',\'toolbar=no,status=no,scrollbars=yes,width=800,height=600\')" value="'.$_language->module['edit'].'" />
	    <input type="button" class="btn btn-danger btn-xs" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'news.php?action=delete&amp;id='.$ds['newsID'].'\')" value="'.$_language->module['delete'].'" />';
		}
		else $adminaction='';
		...
		if(isnewsadmin($userID)) {
			if(!$ds['published']) echo '<form style="display: inline;float: right;" method="post" action="news.php?quickactiontype=publish"><input type="hidden" name="newsID[]" value="'.$ds['newsID'].'" /><input class="btn btn-success btn-xs" type="submit" name="submit" value="'.$_language->module['publish_now'].'" /></form>';
			else echo '<form style="display: inline;float: right;" method="post" action="news.php?quickactiontype=unpublish"><input type="hidden" name="newsID[]" value="'.$ds['newsID'].'" /><input class="btn btn-warning btn-xs" type="submit" name="submit" value="'.$_language->module['unpublish'].'" /></form>';
		}

		$comments_allowed = $ds['comments'];
		$parentID = $newsID;
		$type = "ne";
		$referer = prepareUrl("index.php?site=news_comments&newsID=$newsID");
