...
if(isset($_POST['newreply']) && !isset($_POST['preview'])) {
	...
	if(count($emails)) {
		...
		$link="http://".$hp_url."/".prepareUrl("index.php?site=forum_topic&topic=".$topic);
	...
	nervinc("_attachments");
	$attachments = new attachments($lastpostID, $userID);
	$attachments->setFiles($files);
	$attachments->upload();
	
	header("Location: ".prepareUrl("index.php?site=forum_topic&topic=".$topic."&page=".$page));
	exit();
}
elseif(isset($_POST['editreply']) and (bool)$_POST['editreply']) {
	...
	header("Location: ".prepareUrl("index.php?site=forum_topic&topic=".(int)$_GET['topic']."&page=".(int)$_GET['page']));
}
elseif(isset($_POST['saveedittopic']) and (bool)$_POST['saveedittopic']) {
	...
	header("Location: ".prepareUrl("index.php?site=forum_topic&topic=".$topic));
}
...
function showtopic($topic, $edit, $addreply, $quoteID, $type) {
	...
	if($dt['readgrps'] != "" and !$usergrp and !$ismod) {
		...
		if(!$usergrp){ 
	    	...
	    	redirect(prepareUrl('index.php?site=forum'),$_language->module['no_permission'],2);
	    	return;
		}
	}
	...
	$page_link = '';
	if($pages>1) $page_link = makepagelink(prepareUrl("index.php?site=forum_topic&topic=$topic&type=$type"), $page, $pages);
	if($type=="ASC") {
		$sorter='<a href="'.prepareUrl('index.php?site=forum_topic&topic='.$topic.'&page='.$page.'&type=DESC').'">'.$_language->module['sort'].':</a> <img src="images/icons/asc.gif" alt="" />';
	}
	else {
		$sorter='<a href="'.prepareUrl('index.php?site=forum_topic&topic='.$topic.'&page='.$page.'&type=ASC').'">'.$_language->module['sort'].':</a> <img src="images/icons/desc.gif" alt="" />';
	}
	...
	$topicactions='<a href="printview.php?board='.$dt['boardID'].'&topic='.$topic.'" target="_blank" class="btn btn-default"><i class="fa fa-print"></i></a> ';
	if($loggedin and $writer) $topicactions.='<a class="btn btn-default" href="'.prepareUrl('index.php?site=forum&addtopic=true&action=newtopic&board='.$dt['boardID'].'').'">'.$_language->module['newtopic_image'].'</a> <a  class="btn btn-default" href="'.prepareUrl('index.php?site=forum_topic&topic='.$topic.'&addreply=true&page='.$pages.'&type='.$type.'').'">'.$_language->module['newreply_image'].'</a>';
	...
	$url_forum_categ		 = prepareUrl("index.php?site=forum&cat={$db['category']}");
	$url_forum_board		 = prepareUrl("index.php?site=forum&board={$dt['boardID']}");
	$url_forum_topic		 = prepareUrl("index.php?site=forum_topic&$posttype=$topic");
	...
	elseif($addreply && !$dt['closed']) {
		if($loggedin and $writer) {
			if(isset($_POST['preview'])) {
				...
				if($getavatar = getavatar($userID)) $avatar='<img src="images/avatars/'.$getavatar.'" alt=""  class="img-thumbnail"/>';
				else $avatar='';
	...
	$first_post = safe_query("SELECT * FROM ".PREFIX."forum_posts WHERE topicID='$topic' ORDER BY date ASC LIMIT 1");
	$fist_post_details = mysql_fetch_assoc($first_post);
	$fpID = $fist_post_details['postID'];
	...
	while($dr=mysql_fetch_array($replys)) {
		...
		if($getavatar = getavatar($dr['poster'])) $avatar='<img src="images/avatars/'.$getavatar.'" alt="" class="img-thumbnail" />';
		else $avatar='';
		...
		if($loggedin && $dr['poster']!=$userID) {
			$pm='<a href="'.prepareUrl('index.php?site=messenger&action=touser&touser='.$dr['poster'].'').'"><img src="images/icons/pm.gif" border="0" width="12" height="13" alt="'.$_language->module['messenger'].'" /></a>';
		...
		if(!$dt['closed']) $quote='<a href="'.prepareUrl('index.php?site=forum_topic&addreply=true&board='.$dt['boardID'].'&topic='.$topic.'&quoteID='.$dr['postID'].'&page='.$page.'&type='.$type.'').'"><i class="fa fa-quote-left"></i></a>';
		else $quote = "";
		...
		$actions='';
		if(($userID == $dr['poster'] OR isforumadmin($userID) OR ismoderator($userID,$dt['boardID']))&& !$dt['closed']) $actions=' <a href="'.prepareUrl('index.php?site=forum_topic&topic='.$topic.'&edit=true&id='.$dr['postID'].'&page='.$page.'').'"><i class="fa fa-pencil-square-o"></i></a> ';
		...
		nervinc("_attachments");
		$attachments_class = new attachments($postID);
		$attachments_ctn = $attachments_class->getAll();
		if($attachments_ctn){
			$attachments = "<b>Attachments:</b><br><br><div class='row'>";
			foreach($attachments_ctn as $currAttachment){
				$attachmentSize = $attachments_class->formatSizeUnits(filesize("uploads/attachments/".$currAttachment['filename']));
				$attachments .= "<div class='col-lg-4'><a href='uploads/attachments/".$currAttachment['filename']."'><img class='col-lg-12 img-thumbnail btn btn-default' src='uploads/attachments/".$currAttachment['filename']."' /></a><br><small><i>$attachmentSize</i></small></div>";
			}
			$attachments .= "</div><br><hr>";
		}
		eval ("\$forum_topic_content = \"".gettemplate("forum_topic_content")."\";");
		echo $forum_topic_content;
		unset($actions);
...
//print_r($_SESSION['attachments']['error']);
