...
if(isset($id) and getnickname($id) != '') {
	...
	if($action == "buddys") {
			  ...
			  $url_profile 		= prepareUrl("index.php?site=profile&id=$id");
  $url_galleries	= prepareUrl("index.php?site=profile&id=$id&action=galleries");
  $url_buddys		= prepareUrl("index.php?site=profile&id=$id&action=buddys");
  $url_lastposts	= prepareUrl("index.php?site=profile&id=$id&action=lastposts");
  $url_guestbook	= prepareUrl("index.php?site=profile&id=$id&action=guestbook");
    ...
    $buddys = safe_query("SELECT buddy FROM ".PREFIX."buddys WHERE userID='".$id."'");
        ...
        $buddylist .= '<tr bgcolor="'.$bgcolor.'">
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>'.$country.' <a href="'.prepareUrl('index.php?site=profile&id='.$db['buddy'].'').'"><b>'.$nicknamebuddy.'</b></a></td>
  ...
  $url_guestbook	= prepareUrl("index.php?site=profile&id=$id&action=guestbook");
		...
		if(mysql_num_rows($topics)) {
            ...
            <table width="100%" cellpadding="2" cellspacing="0">
              <tr>
                <td colspan="3"><div style="overflow:hidden;"><a href="'.prepareUrl('index.php?site=forum_topic&topic='.$db['topicID'].'').'">'.$posttime.'<br /><b>'.clearfromtags($db['topic']).'</b></a><br /><i>'.$db['views'].' '.$_language->module['views'].' - '.$db['replys'].' '.$_language->module['replys'].'</i></div></td>
              </tr>
            </table>
            </td>
          </tr>';
		...
		if(mysql_num_rows($posts)) {
            ...
            <table width="100%" cellpadding="2" cellspacing="0">
              <tr>
                <td colspan="3"><a href="'.prepareUrl('index.php?site=forum_topic&topic='.$db['topicID'].'').'">'.$posttime.' <br /><b>'.$db['topic'].'</b></a></td>
              </tr>
              <tr><td></td></tr>
              <tr>
                <td width="1%">&nbsp;</td>
	...
	elseif($action == "guestbook") {
		...
		if(isset($_POST['save'])) {
			...
			redirect(''.prepareUrl('index.php?site=profile&id='.$id.'&action=guestbook').'','',0);
		}
  ...
  $url_guestbook	= prepareUrl("index.php?site=profile&id=$id&action=guestbook");
			...
			if($pages > 1) $page_link = makepagelink(prepareUrl("index.php?site=profile&id=".$id."&action=guestbook&type=".$type), $page, $pages);
			else $page_link='';
			...
			if($type=="ASC")
			$sorter='<a href="'.prepareUrl('index.php?site=profile&id='.$id.'&action=guestbook&page='.$page.'&type=DESC').'">'.$_language->module['sort'].':</a> <img src="images/icons/asc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
			else
			$sorter='<a href="'.prepareUrl('index.php?site=profile&id='.$id.'&action=guestbook&page='.$page.'&type=ASC').'">'.$_language->module['sort'].':</a> <img src="images/icons/desc.gif" width="9" height="7" border="0" alt="" />&nbsp;&nbsp;&nbsp;';
			...
			echo '<form method="post" name="form" action="'.prepareUrl('index.php?site=profile&id='.$id.'&action=guestbook&delete=true').'">';
  ...
  $url_guestbook	= prepareUrl("index.php?site=profile&id=$id&action=guestbook");
		...
		$anzvisits = $ds['visits'];
		if($ds['userpic']) $userpic = '<img src="images/userpics/'.$ds['userpic'].'" alt="" class="img-thumbnail img-circle" />';
		else $userpic = '<img src="images/userpics/nouserpic.gif" alt="" class="img-thumbnail img-circle" />';
		if($ds['userpic']){
			$userpic_url = "images/userpics/".$ds['userpic'];
		}else{
			$userpic_url = "images/userpics/nouserpic.gif";
		}
		...
		if($loggedin && $ds['userID'] != $userID) {
			$pm = '<a href="'.prepareUrl('index.php?site=messenger&action=touser&touser='.$ds['userID'].'').'"><img src="images/icons/pm.gif" border="0" width="12" height="13" alt="messenger" /></a>';
		...
		if(mysql_num_rows($visitors)) {
			...
			while($dv = mysql_fetch_array($visitors)) {
				...
				if(isonline($dv['visitor']) == "offline") $statuspic='<span class="label label-danger">offline</span>';
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td>'.$country.' <a href="'.prepareUrl('index.php?site=profile&id='.$dv['visitor'].'').'"><b>'.$nicknamevisitor.'</b></a></td>
